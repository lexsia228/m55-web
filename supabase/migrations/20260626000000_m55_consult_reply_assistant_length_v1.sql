-- M55 consult reply assistant length bump (SSOT 1,200–1,800 JA target; server cap 2,400)
-- Gate: CATEGORY-1-M55-CONSULT-REPLY-SSOT-UX-LENGTH-PATCH-REV1
...
BEGIN
  ...
  IF p_assistant_message IS NULL OR length(btrim(p_assistant_message)) < 1 OR length(p_assistant_message) > 2800 THEN
    RETURN jsonb_build_object(
      'ok', false,
      'error_code', 'INVALID_ARGUMENT',
      'message', 'assistant_message length must be 1-2800'
    );
  END IF;
...
COMMENT ON FUNCTION public.m55_consult_reply_commit IS
  'Atomically commits consult send. assistant_message max 2800 chars (2026-06-26 SSOT length patch).';
