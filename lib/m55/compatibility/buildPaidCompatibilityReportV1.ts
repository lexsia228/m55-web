...
function perspectiveText(
  ...
  const templates = [
    `${role}には、${perspective}ことが自然に見えます。予定を決める場面では、自分が動ける輪郭を先に確かめたくなります。`,
    `${role}から見ると、${perspective}ところがあります。反応がまだ見えない場面では、受け取った合図の置き方を慎重に扱います。`,
    `${role}には、${perspective}進め方がなじみます。意見が分かれると、話の内容と安心できる順序のどちらを先に置くかが表れます。`,
    `${role}からは、${perspective}流れが自然です。「${topicLabel}」を扱うときも、話題へ入る間合いを自分の順序で整えます。`,
    `${role}には、${perspective}動きが見えます。この場面では、${statusContext}を自分のペースで確かめようとします。`,
    `${role}から見ると、${perspective}ことが戻りやすい入口になります。関係を一つの説明に固定せず、小さな接点から様子を見ます。`,
  ] as const;
