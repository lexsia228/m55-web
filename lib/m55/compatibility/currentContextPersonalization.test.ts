...
describe('current-context questionnaire contract', () => {
  ...
  it('shows trust strip, compressed questionnaire chrome, toolkit tiles, and aligned CTA', () => {
    ...
    assert.match(component, /無料・ログイン不要・6つの質問/);
    ...
    assert.match(component, /あなた側に出やすい反応/);
    assert.match(component, /相手側に表れやすい傾向/);
    assert.match(component, /二人が自然に合いやすいところ/);
    assert.match(component, /互いを補いやすい違い/);
    assert.match(component, /すれ違いが始まりやすい場面/);
    assert.match(component, /選んだテーマについての詳しい結果/);
    assert.match(component, /選んだテーマの詳しい結果を見る/);
