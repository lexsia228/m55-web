...
describe('paid DTR individualization — deterministic DOB layer', () => {
  ...
  it('catalog chapters s1–s3 remain STEM_BODIES-only (individualization does not rewrite them)', () => {
    resetCalendarBundleCacheForTests();
    ...
    for (const sectionId of ['s1_identity', 's2_composition', 's3_essence'] as const) {
  ...
  it('user-facing individualization fragments do not leak internal jargon', () => {
    ...
    assert.ok(ind);
    for (const text of [ind!.auxiliaryReading, ind!.handlingHint]) {
