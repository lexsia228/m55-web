...
> = {
  try: {
    sort: '生年月日の土台では、小さく動かして輪郭を掴んでから、候補を内側で比べやすい基調です。',
...
> = {
  try: {
    try: '土台の始め方も今回の答えも、小さく動かして輪郭を掴む側に重なっています。同じ方向に重なると、前に進んでいるように見えても、試作の点検が内側で続きやすい。',
    map: '土台では小さく動かして輪郭を掴みやすいのに、今回の答えでは全体を揃えてから動く側に寄っています。表では準備してから進む人に見えても、内側では先に一つ試したいが残る。',
    ask: '土台では小さく動かして輪郭を掴みやすいのに、今回の答えでは人に聞いてから動く側に寄っています。相談しているように見えても、自分の中ではもう試作を始めている時間が先行しやすい。',
  },
  map: {
  ...
  ask: {
    try: '土台では周囲の視点を集めてから動きやすいのに、今回の答えでは小さく試して進む側に寄っています。試作している人に見られても、内側ではまだ誰かの見立てが欲しい時間が残る。',
...
> = {
  ...
  solo: {
    ...
    solo: '土台の距離の取り方も今回の答えも、一人の時間で整えてから戻る側に重なっています。同じ方向に重なると、会食や会議のあとに一人で「今の決め方でよかったか」を見直しやすい。',
  },
};
...
> = {
  ...
  adjust: {
    ...
    rebuild: '土台では差分修正で進みやすいのに、今回の答えでは前提から組み直す側に寄っています。周囲には急な方向転換に見え、本人の中では細部を直していた延長で一貫している。',
  },
  rebuild: {
