...
> = {
  try: {
    try: {
      ...
      manifestationJa:
        '小さく試し始めたあとも、まだ決め切れていないまま動いている。動いている人に見られやすいが、実際に確定するのは、一人でやり直したあとだ。新しいことを一つ動かした夜、一人でやり直し直したくなる。',
...
> = {
  close: {
    close: {
      shortJa: '近さを言葉にした直後に言い方をやり直す',
      manifestationJa:
        '会話の中で近さを言葉にした直後に、言い方を一人でやり直している。丁寧に見えるほど、「今さら考え直している」と見られやすい。',
      beatJa: '近さを言葉にした直後に、言い方を一人でやり直している。',
      sceneWhen: '近い人と話したあと、決めたことを送る前に一度止まる。',
      sceneWhile: '近さを言葉にした直後に、言い方をやり直したくなる',
    },
    middle: {
      shortJa: '会話では合わせて、帰宅後に返事の間を自分のペースに戻す',
      manifestationJa:
        '会話では合わせているように見られやすい。会ったあとに、返事のタイミングを一人で戻したくなる。近い人と話したあと、一日の終わりに返事の間を自分のペースに整える。',
      beatJa: '会話では合わせ、帰宅してから返事の間を自分のペースに戻している。',
      sceneWhen: '近い人と話したあと、一日の終わりに返事の間が気になる。',
      sceneWhile: '会ったあと、返事の間を一人で整えたくなる',
    },
    solo: {
  ...
  middle: {
    close: {
      shortJa: '普段は間を取り、会話では近さを言葉にして伝える',
      manifestationJa:
        '普段は間を取っているのに、会話の中では近さを言葉にして伝えてしまう。丁寧に見える一方、あとから「近づきすぎた」が残る。普段は間を取っている相手に、会話の途中で近さを言葉にする。',
      beatJa: '普段は間を取りつつ、会話では近さを言葉にして伝えてしまう。',
      sceneWhen: '誰かと話したあと、会話の途中で近さを言葉に整えたくなる。',
      sceneWhile: '会話の途中で、近さを言葉にして伝えてしまう',
    },
    middle: {
      shortJa: '連絡は続く夜、迷いだけ一人で抱える',
      manifestationJa:
        '安定して見えるほど、迷いを一人で抱えたまま連絡だけ続けている。離れていないように見られても、返事の言い方は会話のあとに一人で直したくなる。',
      beatJa: '連絡は続く夜、迷いだけ一人で抱えている。',
      sceneWhen: '連絡は続いている夜、一日の終わりに返事の言い方を直したくなる。',
      sceneWhile: '連絡は続けつつ、会話のあとに一人で返事を直したくなる',
    },
    solo: {
  ...
  solo: {
    close: {
      ...
      manifestationJa:
        '一人で整えてから戻りたいのに、その場では近さを言葉にして合わせている。近い関係を保っているように見られても、帰宅後に一人の時間が先に立つ。',
...
> = {
  ...
  adjust: {
    ...
    rebuild: {
      shortJa: '小さく直すたびに、周囲には方向転換に見える',
      manifestationJa:
        '小さく直すたびに、周囲には方向転換に見えやすい。本人の中では、一度止めてから手順を入れ替えている。予定を変えるか迷うとき、止めてから手順を入れ替え始める。',
      beatJa: '小さく直すたびに、本人は一度止めてから手順を入れ替えている。',
      sceneWhen: '予定を変えるか迷うとき、止めてから手順を入れ替え始める。',
      sceneWhile: '迷っているとき、止めてから手順を入れ替え始める',
    },
  },
  rebuild: {
    observe: {
      shortJa: '方針を変える前に、一日置いてから動き出す',
      manifestationJa:
        '組み直したいのに、表では一日置いている。静かに見える一方、前提が変わったことはもう決めている。方針は決めたまま、表では一日置く。',
      beatJa: '方針は決まっていて、表では一日置いている。',
      sceneWhen: '予定の前提が変わった週に、動き出す前に一日置きたくなる。',
      sceneWhile: '方針は決まっていて、表では一日置く',
    },
    adjust: {
      shortJa: '小さく直している人に見えて、本人は一度やり直すつもり',
      manifestationJa:
        '小さく直している人に見られても、本人は一度やり直すつもりで手を動かしている。表の小さな直しと、本人のやり直しは同じに見えない。',
      beatJa: '小さく直して見えても、本人はやり直すつもりで動かしている。',
      sceneWhen: '予定を変えるか迷う夜、手を動かし始める。',
      sceneWhile: '小さく直して見えても、やり直すつもりで動かしている',
    },
    rebuild: {
      shortJa: '前提が崩れたときの見直しが急に見える',
      manifestationJa:
        '予定が崩れたときに、周囲には急な方向転換に見える見直しが入る。本人の中では「前提が変わったから」で一貫している。予定が崩れた夜、周囲には急な見直しに見える。',
      beatJa: '予定が崩れたときの見直しが、周囲には急に見えやすい。',
      sceneWhen: '予定が崩れた夜、周囲には急な見直しに見える。',
      sceneWhile: '予定が崩れたとき、急な見直しに見えやすい',
    },
  },
};
...
export function premiumBridgeForManifestation(
  ...
  const profileCue =
    birth && birth.start !== answers.start
      ? answers.start === 'try'
        ? '手を出してから整える流れで'
        : answers.start === 'map'
          ? '段取りを見てから動く流れで'
          : '相談してから動く流れで'
      : answers.change === 'rebuild'
        ? '前提を見直す流れで'
        : answers.change === 'observe'
          ? '変化のあと様子を見る流れで'
          : answers.decision === 'wait'
            ? '一度置いてから返す流れで'
            : answers.decision === 'deadline'
              ? '「ここまで」が見える前の流れで'
              : '決める前に並べ直す流れで';
  if (axisId === 'distance') {
    const context =
      answers.distance === 'solo'
        ? '会ったあと、一人の時間で整えてから戻るとき'
        : answers.distance === 'close'
          ? '近い関係で返事の間を保つとき'
          : '連絡は続けつつ、返事の間だけ自分のペースに戻したいとき';
    return `いま見えた「${shortJa}」は、${context}、${profileCue}先に出やすい。プレミアムでは、その続きを仕事と関係に分けて読めます。`;
  }
  if (axisId === 'change') {
    const context =
      answers.change === 'rebuild'
        ...
        : answers.change === 'observe'
          ? '変化の直後に一日様子を見るとき'
          : '変わったところだけ直して進めるとき';
    return `いま見えた「${shortJa}」は、${context}、${profileCue}形を変えやすい。プレミアムでは、その続きを場面ごとに読めます。`;
  }
  if (axisId === 'recovery') {
    const context =
      answers.recovery === 'scene'
        ? '場所や空気を変えてから戻るとき'
        : answers.recovery === 'shrink'
          ? 'やることを狭めて立て直すとき'
          : '短い休みを入れて戻るとき';
    return `いま見えた「${shortJa}」は、${context}、${profileCue}負荷になりやすい。プレミアムでは、楽に戻す順を読めます。`;
  }
  if (axisId === 'decision') {
    const context =
      answers.decision === 'wait'
        ...
        : answers.decision === 'deadline'
          ? '「ここまで」が見えたところで決めるとき'
          : '候補を並べてから一つに絞るとき';
    return `いま見えた「${shortJa}」は、${context}、${profileCue}力にも遅れにもなります。プレミアムでは、その続きを読めます。`;
  }
  const context =
    answers.start === 'map'
      ? '全体の段取りが見えてから動くとき'
      : answers.start === 'ask'
        ? '周りの意見を聞いてから取りかかるとき'
        : '小さく一つ動かしてから様子を見るとき';
  return `いま見えた「${shortJa}」は、${context}、${profileCue}武器にも摩擦にもなります。プレミアムでは、その続きを読めます。`;
}
