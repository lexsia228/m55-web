...
> = {
  ...
  deadline: {
    ...
    adjust: '基準を置いて決めたあと、変更点は小さく直していく。期限で閉じたように見えても、差分修正が内側で続きやすい。',
...
> = {
  try: {
    sort: '仕事では、試作を一つ出した直後に比較表を作り直す場面で出やすい。前に進んだように見えても、候補の並べ直しが終わっていない。',
    deadline: '仕様が固まっていない課題で小さく試しつつ、「金曜までに決める」が先に立つ場面で出やすい。試している途中で結論側へ寄る。',
  ...
  ask: {
    ...
    deadline: '共有を足して材料を増やしつつ、期限が来ると一気に閉じる場面で出やすい。開いていた会議が、締切で急に終わる。',
    wait: '仕事や判断では、人に聞いて材料を増やしたあと、結論は置いてから返す場面で出やすい。聞いているうちに決まったように見られても、本人はまだ閉じていない。',
  },
};
...
const RELATION_SCENE: Readonly<Record<DistanceTendency, string>> = {
  ...
  solo: '人との距離では、会食や会議のあとに一人の時間を先に確保し、そこで「今の決め方でよかったか」を見直す場面が典型です。',
};

const CHANGE_SCENE: Readonly<Record<ChangeTendency, string>> = {
  observe: '予定変更の直後にすぐ組み替えず一日観察する場面で、表の静けさと内側の再点検が同時に出やすいです。',
  adjust: '変更点だけを差分修正して全体は崩さない場面で、方針は守っているのに細部の点検が長く残りやすいです。',
...
> = {
  try: {
    ...
    deadline: ['試す余白と、決める区切りが両方見えるとき', '途中の試作が失敗扱いにならないとき', '締切の前に一度だけ見直す時間が取れるとき'],
    wait: ['動き出しと結論の時間を分けてよいとき', '即答を急かされないとき', '試した事実と、まだ決めていないことが両方言えるとき'],
  },
  map: {
    sort: ['全体像を共有したあと、比較に入る時間が残っているとき', '準備と決定を別の段に置けるとき', '候補を減らす合意が先に取れるとき'],
...
> = {
  close: {
    observe: ['境界が曖昧なまま関わりが続くとき', '様子を見る前に即応だけを求められるとき', '言葉にした距離が「拒否」に読まれるとき'],
    adjust: ['境界が曖昧なまま関わりが続くとき', '小さな修正の余地なく大転換だけを求められるとき', '近い関係で再点検が「今さら」に見えるとき'],
    rebuild: ['境界が曖昧なまま関わりが続くとき', '継ぎ足しだけで前提の更新が許されないとき', '組み直しが相手への急変に見えるとき'],
  },
  middle: {
...
function premiumOpenQuestion(axes: ExpressionAxes): string {
  ...
  return `${live}は同じ動きがつながるのに、${heavy}にその動きが止まります。この差が、距離の整え方と変化の最初の一手のどちらから来るのかは、場面を分けて見ないと決まりません。`;
}
