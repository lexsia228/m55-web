...
export async function getCreatorPortal(userId: string) {
  ...
  const [apps, profile] = await Promise.all([
    db.from('m55_creator_applications')
      .select('id,status,application_source,source_campaign,terms_version,terms_accepted_at,submitted_at,rejected_reapply_after')
  ...
  return { application: apps.data?.[0] ?? null, profile: profile.data ?? null };
}
