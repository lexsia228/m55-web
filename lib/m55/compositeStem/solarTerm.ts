import { lookupSolarTermsForYear } from '../calendar/loadCalendarBundle';
...
export function resolveSolarTermMetadata(
  effectiveLocalDate: string,
  birthTime: string,
  offsetMinutes: number,
  solarYearKey: number,
): Pick<BoundaryMetadata, 'solarTermKey' | 'solarTermBoundaryInstant' | 'solarYearKey'> {
  const yearRow = lookupSolarTermsForYear(solarYearKey);
  const alsoPrev = lookupSolarTermsForYear(solarYearKey - 1);
