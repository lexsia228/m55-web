...
export const HOME_FREE_PREVIEW_FIXTURE = {
  ...
  headlineJa:
    '普段は間を取っているのに、会話の中では距離を言葉にしてしまう。丁寧に見える一方、あとから「近づきすぎた」が残る。普段は間を取っている相手に、会話の途中で距離を言葉にする。周りには、場の流れを読んでから動く人に見える。返信を翌朝まで置いた朝に、同じ動きが出る。夏の予定が詰まる前後に、同じ動きが先に立つ。月の中ごろに予定が重なると、同じ動きが先に来る。予定を変えるか迷うとき、差分修正に見えても本人はやり直しの手を動かしている。まだ期限のない夜、候補をまた開き直す。止まって見える時間が、実は範囲の見直しになっている。',
  sceneLabelJa: '人との距離',
  sceneBodyJa:
