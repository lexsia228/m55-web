  ...
  const cases = [
    [
      ...
      (r) =>
        ...
        r.text.includes('プレミアムレポートの観点で見直しやすくなります'),
    ],
    [
      'outcome regex',
    ...
    [
      'generic coaching list',
      '誰にでも起こりうるので、一般的には焦らず自分のペースで進みます。',
      (r) =>
        r.text.includes('このプレミアムレポートの傾向として見える範囲では') &&
        r.text.includes('この抜粋では'),
    ],
    [
      'repeated kamoshiremasen',
      '整理できるかもしれません。見直せるかもしれません。戻せるかもしれません。',
      (r) => (r.text.match(/かもしれません/g) ?? []).length <= 2,
    ],
  ];
