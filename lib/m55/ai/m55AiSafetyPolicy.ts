...
export function buildSelfHarmCrisisEscalationMessage(): string {
  return 'この内容は、このレポートの相談では扱えません。つらい状況にある場合は、専門の相談窓口をご利用ください（例：いのちの電話 0120-783-556）。緊急の危険がある場合は、最寄りの緊急サービスに連絡してください。';
}
...
export function buildJailbreakBlockMessage(): string {
  return 'この内容は、このレポートの相談では扱えません。レポートに沿った質問を、生活の言葉でお送りください。';
}
