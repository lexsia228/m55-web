...
const { home, howM55Works } = TOP_FREE_ENTRY_PUBLIC_COPY;

export function FrameworkSection() {
  return (
    <section className={`${styles.shellWide} ${styles.foldAlt}`} aria-labelledby="how-m55-framework-title">
      <p className={styles.sectionKicker}>02 — {home.exploreQualitiesTitleJa}</p>
      <h2 id="how-m55-framework-title" className={styles.visuallyHidden}>
        {home.exploreQualitiesTitleJa}
      </h2>

      <div className={styles.frameworkGrid}>
        <div className={styles.frameworkRow}>
          <div>
            <p className={styles.sectionLead}>最初に見えるのは、10資質レーンです。</p>
            <p className={styles.sectionLead}>
              ただし、これは全体を決める答えではなく、読み始めるための入口です。
            </p>
            <p className={styles.sectionLead}>{home.fiveAxisQualitiesNoteJa}</p>
            <p className={styles.sectionLead}>
              そのため、同じ称号に見えても、受け取る内容は一人ずつ変わります。似た入口を持つ人がいても、見取り図や
              {howM55Works.frameworkSavedJa}
            </p>
          </div>
          <div className={styles.frameworkVisual}>
            ...
            <p className={styles.visualCaption}>10資質レーンのビジュアル</p>
          </div>
        </div>
      </div>
    </section>
