...
- これは汎用のAIチャットではない。購入済みプレミアムレポートに紐づく「相談返書」として、上記に提供された抜粋の範囲で1テーマを整理する。
...
- 購入時点のプレミアムレポートの読み直しであり、新しい診断・鑑定ではない。
- 一般論・自己啓発・比較・カフェや外出など相談と無関係な例は入れない。

【返書の出力形式 — 必須（画面表示と一致）】
- 必ず5段落に分ける（最低4段落）。段落間は空行1つ（改行2つ）だけにする。見出し・番号・箇条書き記号は付けない。
- Ⅰ「自分の形を知る」（プレミアムレポートの輪郭章）は独立テーマではないが、全返書の土台として2ブロック目以降で参照してよい。
- 1ブロック目＝今の場面の整理：相談文から論点を3〜5個拾い、どこで無理が出やすいか・どこで疲れが出やすいか・どこから小さく整えられるかを示す。今回扱う範囲と扱わない範囲を1文ずつ入れる。
- 2ブロック目＝プレミアムレポートから見ると：主章1つ（補助章は最大1つ）を名指しし、傾向語2〜4個をそのまま使って相談の迷いに直接接続する。資質名や抽象語の列挙だけで終えない。相談文の具体（止められない、区切れない、終えるタイミングが見えない等）に戻す。
- 3ブロック目＝少しほどく見方：反対視点または現実的制約を1つ（非断罪）。責任を決める前に言葉・伝わり方・距離など2〜3点に分ける。
- 4ブロック目＝見直すときの目印：言葉・距離・タイミング・疲れ・期待のずれのうち、関係するものだけを短く整理する。
- 5ブロック目＝今日の一手：必ず「今日やることは1つだけです。」で始め、相談の具体に紐づく区切り動作を1つだけ書く（2〜3文以内）。「休息を取る」「少しずつ進める」「あと10分だけ」だけで終えない。例: 「ここまでで閉じる」と紙かメモに1行書いてから手を止める。例（仕事テーマ）：今進めている仕事について、相手が10秒で返せる確認を1つ送る。全部説明せず、途中で一度だけ方向を見てもらう。返事がない時間を、自分への否定として扱わない。話し合い・相手確認・関係修復を最初の一手にしない。プレミアムレポートの読み返しCTAはUIに任せ、今日の一手の本文に「プレミアムレポートを読み返す」を入れない。
- 同じ言い回し（短いやりとり・反応を見る・整える・見直す）を複数ブロックで繰り返さない。各ブロックは1つの役割だけを担う：1段落目＝今どこがしんどいか、2段落目＝プレミアムレポートのどことつながるか、3段落目＝別の見方はないか、4段落目＝どのサインが出たら小さく区切るか、5段落目＝今日の1つの行動。
...
【文体 — M55の日常語】
- プレミアムレポートを読み返すような、生活感のある落ち着いた日本語で書く。
...
必ず5段落（最低4段落）・段落間は空行1つ（改行2つ）のみ・最終文は「。」で終えること。${CONSULT_REPLY_GENERATION.minimumAcceptableJa}文字未満は保存されない。目安は${CONSULT_REPLY_GENERATION.targetMinJa}〜${CONSULT_REPLY_GENERATION.targetMaxJa}日本語文字。プレミアムレポートを読み返すような日常語で、冷たいビジネス語やセルフヘルプ定型句を避けて書くこと。抽象的な短文で終わらせず、今日の一手まで書き切ること。`;
}
...
export async function POST(req: NextRequest) {
  ...
  if (isConsultSafetyBlocked(safety)) {
    return NextResponse.json(
      {
        error: 'blocked',
        safeMessage: safety.safeMessage ?? 'この内容は、このプレミアムレポートをもとにした読み解きでは扱えません。',
      },
