...
- 2ブロック目＝保存版から見ると：主章1つ（補助章は最大1つ）を名指しし、傾向語2〜4個をそのまま使って相談の迷いに直接接続する。資質名や抽象語の列挙だけで終えない。相談文の具体（止められない、区切れない、終えるタイミングが見えない等）に戻す。
- 3ブロック目＝少しほどく見方：反対視点または現実的制約を1つ（非断罪）。責任を決める前に言葉・伝わり方・距離など2〜3点に分ける。
- 4ブロック目＝見直すときの目印：言葉・距離・タイミング・疲れ・期待のずれのうち、関係するものだけを短く整理する。
- 5ブロック目＝今日の一手：必ず「今日やることは1つだけです。」で始め、相談の具体に紐づく区切り動作を1つだけ書く（2〜3文以内）。「休息を取る」「少しずつ進める」「あと10分だけ」だけで終えない。例: 「ここまでで閉じる」と紙かメモに1行書いてから手を止める。例（仕事テーマ）：今進めている仕事について、相手が10秒で返せる確認を1つ送る。全部説明せず、途中で一度だけ方向を見てもらう。返事がない時間を、自分への否定として扱わない。話し合い・相手確認・関係修復を最初の一手にしない。保存版の読み返しCTAはUIに任せ、今日の一手の本文に「保存版を読み返す」を入れない。
