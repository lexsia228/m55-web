...
- 5ブロック目（任意）＝今日の一手：必ず「今日やることは1つだけです。」で始め、相談の具体に紐づく行動を1つだけ書く。例（仕事テーマ）：今進めている仕事について、相手が10秒で返せる確認を1つ送る。全部説明せず、途中で一度だけ方向を見てもらう。返事がない時間を、自分への否定として扱わない。話し合い・相手確認・関係修復を最初の一手にしない。保存版の読み返しCTAはUIに任せ、今日の一手の本文に「保存版を読み返す」を入れない。
- 4ブロックのみのときは4ブロック目に「今日の一手」を含めてよい。同じ言い回し（短いやりとり・反応を見る・整える・見直す）を複数ブロックで繰り返さない。各ブロックは1つの役割だけを担う：1段落目＝今どこがしんどいか、2段落目＝保存版のどことつながるか、3段落目＝別の見方はないか、4段落目＝どのサインが出たら小さく区切るか、5段落目＝今日の1つの行動。
...
必ず4〜5ブロック・ブロック間は空行1つのみ。保存版を読み返すような日常語で、冷たいビジネス語やセルフヘルプ定型句を避けて書くこと。`;
}
