...
export default function CreatorAffiliateTermsPage() {
  return (
    <main style={mainStyle}>
      ...
      <section style={sectionStyle} aria-labelledby="creator-start-guide">
        <h2 id="creator-start-guide" style={h2Style}>Creator向け開始前ガイド</h2>
        <p style={pStyle}>
          このガイドは、Creator が紹介を始める前に確認しやすいよう、本規約の要点を平易にまとめた案内です。
          契約条件は以下の各条が正準であり、このガイドが各条の内容を変更するものではありません。
        </p>
        <ul style={{ ...ulStyle, marginBottom: 10 }}>
          <li>
            <strong>対象商品：</strong>
            M55 プレミアムレポート ライト（¥1,000）と M55 プレミアムレポート フル（¥1,480）の対象購入です。
          </li>
          <li>
            <strong>Founding Creator：</strong>
            初回最終承認日時を起点に、180日未満は50%、180日以上365日未満は40%、365日以上は30%です。
            同一 Creator の経済条件を、紹介URLやアカウント等の変更でリセットすることはできません。
          </li>
          <li>
            <strong>アトリビューション：</strong>
            30日間のうち、最後の有効な直接 Creator 接触が対象です。1件の購入につき報酬対象は最大1名です。
          </li>
          <li>
            <strong>報酬の見え方：</strong>
            PENDING は標準審査中、HOLD は確認事項があり支払判断を保留している状態、
            PAYABLE は審査を終え支払条件の判定対象となる状態を示すための表示です。
            取消・調整・相殺等は第7条・第8条に従います。
          </li>
          <li>
            <strong>標準支払：</strong>
            支払可能残高20,000円以上を基準に月次処理し、原則として締切月の翌月15日が支払期日です。
            支払処理手数料は第11条の算定式に従います。
          </li>
        </ul>

        <p style={pStyle}>
          本人確認、振込先情報、資格・コンプライアンス等に確認事項がある場合、支払処理が HOLD になることがあります。
          その場合、Creator は当社から求められた情報を確認・提出し、必要に応じて
          <Link href="/support">サポート窓口</Link>
          へ連絡してください。当社は確認理由と必要な対応を案内し、確認後に支払可否を再判定します。
        </p>

        <p style={pStyle}>
          M55 は、医療上の診断、心理検査、治療・カウンセリング、将来の確実な予測、法律・投資上の助言を提供するものではありません。
          一つの情報だけで人や関係を決めつけず、自己理解や関係性を読み解くための参考情報として紹介してください。
        </p>

        <p style={pStyle}>
          紹介時は、投稿の冒頭など利用者が気づきやすい位置に「PR」「広告」「アフィリエイト」等を明示してください。
          以下は、そのまま調整して使える紹介文の例です。
        </p>
        <ul style={{ ...ulStyle, marginBottom: 10 }}>
          <li>
            <strong>短文例：</strong>
            「PR｜M55は、自分や関係性を決めつけずに読み解くための参考情報です。Creator Affiliateとして紹介しています。」
          </li>
          <li>
            <strong>商品紹介例：</strong>
            「広告｜M55 プレミアムレポートは、無料面と同じ土台をもとに、購入時点の入力内容を読み返しやすく整理したウェブ上のレポートです。アフィリエイトリンクを含みます。」
          </li>
        </ul>

        <p style={pStyle}>
          報酬率は収入の保証ではありません。実際の報酬は、有効な直接紹介による対象購入、審査、返金・チャージバック、
          取消・調整等の影響を受けます。収益額や購入結果を保証する表現は使用しないでください。
        </p>

        <p style={pStyle}>
          Founding Creator は、本プログラム初期段階の承認済み Creator に適用する料率区分の呼称です。
          Creator の募集や下位組織への報酬、参加費、必須購入、投稿ノルマはありません。
          また、本規約が公開されていること自体は、Creator 申請受付、一般公開、報酬支払の開始を意味しません。
        </p>
      </section>
