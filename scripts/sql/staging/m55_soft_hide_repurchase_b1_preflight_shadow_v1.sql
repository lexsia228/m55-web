-- CORE-DTR-SOFT-HIDE-REPURCHASE-B2-FIX-B — Staging preflight (m55-soul-shadow) READ ONLY
...
-- Forbidden column names in DML strings: engine_context (use engine_context_json only), product_label.
...
-- Schema-only metrics (never touch dtr_report_snapshots columns directly)
...
-- Table-backed metrics via dynamic SQL only (whitelist: user_id, product_id, engine_context_json, engine_version)
DO $$
DECLARE
  v_count bigint;
