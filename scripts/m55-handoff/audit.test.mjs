...
import { exitCodeForStatus, htmlEscape, parseWorktrees, redactRemote, resolveGitExecutable, safeRead, stableJson, validateAuthorityPath } from './engine.mjs';
import { AUTHORITY_MANIFEST, inspectM55 } from './m55-adapter.mjs';
import { inspectNebula } from './examples/nebula-adapter.mjs';
import { renderHtml } from './packet.mjs';
...
test('18 deterministic symlink escape rejection without filesystem privilege', () => assert.throws(() => validateAuthorityPath('/repo', '/repo/authority.md', '/outside/authority.md', 'authority.md'), /AUTHORITY_SYMLINK_ESCAPE/));
test('18b real symlink escape rejection where supported', (t) => { const root = fixture(); const outside = path.join(os.tmpdir(), `m55-outside-${Date.now()}`); fs.writeFileSync(outside, 'token-like-value'); try { fs.rmSync(path.join(root, 'AGENTS.md')); fs.symlinkSync(outside, path.join(root, 'AGENTS.md')); assert.throws(() => safeRead(root, 'AGENTS.md'), /AUTHORITY_SYMLINK_ESCAPE/); } catch (error) { if (error.code === 'EPERM') t.skip('host does not permit symlink creation'); else throw error; } finally { fs.rmSync(root, { recursive: true, force: true }); fs.rmSync(outside, { force: true }); } });
test('19 stable JSON ordering', () => assert.equal(stableJson({ z: 1, a: { y: 2, b: 3 } }), '{"a":{"b":3,"y":2},"z":1}'));
test('20 usage error exit 2', () => assert.equal(spawnSync(process.execPath, ['scripts/m55-handoff/audit.mjs'], { cwd: ROOT, env: process.env, shell: false, windowsHide: true }).status, 2));
...
test('judge mode writes canonical five-scenario packets with full HTML contract', () => { assert.equal(spawnSync(process.execPath, [path.join(ROOT, 'scripts/m55-handoff/demo.mjs')], { env: process.env, shell: false, windowsHide: true }).status, 0); const out = path.join(os.tmpdir(), 'm55-control-plane-demo'); for (const name of ['CLEAN_READY', 'DOCUMENTED_TRANSITION_WARNING', 'DIRTY_HOLD', 'UNEXPLAINED_DRIFT_HOLD', 'MISSING_AUTHORITY_HOLD']) { const html = fs.readFileSync(path.join(out, name, 'handoff-report.html'), 'utf8'); assert.match(html, /Next single action:|Prohibited next action:|Control Plane · evidence|Commit:/); assert.ok(fs.existsSync(path.join(out, name, 'agent-bootstrap.txt'))); } assert.match(fs.readFileSync(path.join(out, 'DIRTY_HOLD', 'agent-bootstrap.txt'), 'utf8'), /^HOLD — DO NOT BEGIN IMPLEMENTATION\.$/m); });
test('Windows Git resolver honors PATH and PATHEXT', () => { const found = resolveGitExecutable({ platform: 'win32', env: { PATH: 'C:\\Program Files\\Git\\cmd', PATHEXT: '.EXE;.CMD' }, existsSync: (candidate) => candidate.endsWith('git.exe') }); assert.equal(found, path.win32.join('C:\\Program Files\\Git\\cmd', 'git.exe')); });
test('canonical HTML escapes injected values', () => { const html = renderHtml({ status: 'HOLD', reasonCodes: ['<script>'], generatedAt: '2026-01-01T00:00:00.000Z', checks: [], repository: { head: '<script>' }, authority: { nextSingleAction: '<script>', prohibitedLanes: ['<script>'] } }); assert.ok(!html.includes('<script>')); assert.match(html, /&lt;script&gt;/); });
