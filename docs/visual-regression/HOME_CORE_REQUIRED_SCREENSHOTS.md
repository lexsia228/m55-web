...
| 3 | `03-core-locked.png` | `/core` 未保存 | 常時入力フォームが**ない**か。静かな案内面＋**単一**CTA「ホームへ戻る」（`/home`）。プロフィール入力は home の無料導線で完結 |
...
3. **`/core` 未保存**: ストレージクリア → `/core` → 案内＋「ホームへ戻る」のみ。`/home` で無料導線から保存 → 解析中 → `/core`（SoulBirthGate が勝手に被さっていないこと）。
...
- [ ] `/core` 未保存は案内＋**単一**CTA「ホームへ戻る」（`/core` では入力完結させない）
