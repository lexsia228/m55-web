...
Audience: a **new ChatGPT thread with no access to the prior conversation**.
...
This document must be sufficient on its own. Do not treat the old chat conversation as authority above the facts recorded here and in committed SSOT.
...
| New docs-only transition (WT-012) | `/Users/lexsia/Documents/M55_WORKTREE-pr81-post-merge-transition-v1` (`chore/m55-pr81-post-merge-transition-v1`) | **ACTIVE (docs-only)** | This handoff's worktree. No deletion of either retained worktree above until new-thread verification passes. |
...
1. Read `AGENTS.md` and the SSOT in the required order (`AGENTS.md` → `.product-authority/generated/authority-header.md` → `docs/ssot/README.md` → `M55_CURRENT_STATE.md` → `M55_WORKTREE_REGISTRY.md` → `M55_COMMERCIAL_FUNNEL_SSOT.md` → `M55_COMMERCIAL_QUALITY_CONTRACT.md` → active lane contract → `M55_DECISION_LOG.md` → `M55_ROADMAP.md`).
...
## Known generator lag (non-blocking, record only)

`.product-authority/generated/authority-header.md` and `.product-authority/observations.json` still show pre-merge values as of their last generation (`2026-07-27T09:56:00+00:00`): `last observed origin/main SHA: 696559009367a6ac445dc7a07876590b16cd8488`, `production.lastObservedSha: null`, `Growth Share (WT-011): ACTIVE`, `PR #81: OPEN_UNMERGED_BRANCH_LOCAL`. This is **generator-owned lag**, not a contradiction of the Git/CI/Production facts recorded in this document. Do not manually edit `.product-authority/generated/**` to close this lag — it requires a separate Product Authority Pack reconciliation run, which is out of scope for this docs-only patch.
