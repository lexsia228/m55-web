...
**CURRENT (2026-08-14 post-preflight) — authoritative, read this first:** Production diagnostics @ `0b2b2b349ccf4f0b9549d5a59be5a92be0a99578`. PR #117 product @ `be6efb4fd7b2994a18fe0f175a536e773ee827ce`. Four-surface visual identity **CLOSED GREEN**. Personal Free / Personal Premium **CLOSED GREEN**. Compatibility Free **CLOSED GREEN**. `M55_LIVE_UPGRADE_RPC_V2_SAME_TRANSACTION_RECOVERY` **CLOSED_GREEN**. Pair Premium **READY_BUT_GATED**. Compatibility commerce **OFF**. ACTIVE branch: `fix/m55-compatibility-commerce-preflight-v1` (read-only precheck/postcheck SQL + SSOT). Stripe LIVE compatibility binding **NO_MATCH_CREATE_REQUIRED**. **NEXT GATE:** `M55_PAID_COMPATIBILITY_PRODUCTION_DB_PRECHECK` (Human SQL Editor · no migration apply).
